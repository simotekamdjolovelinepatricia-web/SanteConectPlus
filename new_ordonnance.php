...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($consultationId) || empty($medicament) || empty($dosage)) {
        $erreur = "Les champs obligatoires doivent être remplis.";
    } else {
        try {
            $stmt = $conn->prepare("
                INSERT INTO ordonnance (medicament, dosage, duree, instructions, dateOrdonnance, idConsultation)
                VALUES (?, ?, ?, ?, CURDATE(), ?)
            ");
            $stmt->execute([$medicament, $dosage, $duree, $instructions, $consultationId]);
...
<head>
    ...
    <title>Créer Ordonnance | Santé Connect+</title>
    <style>
    ...
    <div class="container">
        <div class="form-card">
            ...
            <?php if ($success): ?>
                ...
                <p><a href="new_ordonnance.php" class="back-link">Créer une autre ordonnance</a> | 
            ...
            <?php else: ?>
                ...
                <form method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="medicament" required placeholder="Nom du médicament">
                    </div>

                    <div class="form-row">
                        <div class="form-group">
                            <label>Dosage *</label>
                            <input type="text" name="dosage" required placeholder="Ex: 500mg">
                        </div>
                        <div class="form-group">
                            <label>Durée du traitement</label>
                            <input type="text" name="duree" placeholder="Ex: 7 jours">
                        </div>
                    </div>

                    <div class="form-group">
                        <label>Instructions</label>
                        <textarea name="instructions" placeholder="Instructions de prise (Ex: 3 fois par jour avec les repas)"></textarea>
                    </div>
